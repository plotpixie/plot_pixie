...
void main() async {
  ...
  final List<Node> darkSatires = await Pixie().getIdeas(
      "a dark satire combining a beloved children franchise with a more adult-oriented concept it is not normally associated with");
  darkSatires.forEach((book) {
    print(book.toJson());
  });

  List<Node> characters = await Pixie()
      .getCharacterSuggestions(darkSatires[0], numberOfCharacters: 5);
  characters.forEach((character) {
    print(character.toJson());
  });

  List<Node> acts = await Pixie().getOutline(darkSatires[0], characters);
  acts.forEach((act) {
    print(act.toJson());
  });
  ...
  String newScene = await Pixie().transformText(scene,
      'make it longer, funnier and add some references to a unknown cosmic horror');
