...
class Prompter {
  ...
  static String decoratePrompt(String text, String returnType,
    ...
    if (returnType == 'Node') {
      ...
      decorated += "${_isArray( _getCharacterRepresentation(), isArray)}. Give them a name and short blurb and 8 random unique traits specific to the character. Trait options are a random item from $options . Be specific. Instead of <character> has a habit of humming to herself when she's happy in the trait details prefer shorter sentences like hums to herself when happy ";
    }
    return decorated;
  }
...
var    prompt = Prompter.decoratePrompt("We're in a far away galaxy where people are human dinosaur hybrids, tell me about 4 different characters in the story.", "Character", isArray:true);
    print(prompt);
    var parsedResult = Prompter.convertResult(await Gemini().prompt(prompt), "Character", true);
